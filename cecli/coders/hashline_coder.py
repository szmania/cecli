...
class HashLineCoder(Coder):
    """A coder that uses hashlines blocks for code modifications."""

    edit_format = "hashline"
    prompt_format = "hashline"
    hashlines = True
    ...
    def apply_edits(self, edits, dry_run=False):
        ...
        # Group hashline edits by file path
        hashline_edits_by_file = {}
        original_hashline_edits_by_file = {}
        regular_edits = []

        for edit in edits:
            path, original, updated = edit

            # Check if this is a hashline edit (list with 3 elements)
            is_hashline = False
            if isinstance(original, list):
                # Check if this is the original format: [start_hash, end_hash, operation]
                if len(original) == 3 and all(isinstance(item, str) for item in original):
                    ...
                    if operation in ["replace", "insert", "delete"]:
                        ...
                        if (isinstance(start_hash, str) and "|" in start_hash) and (
                            ...
                            if path not in hashline_edits_by_file:
                                ...
                                original_hashline_edits_by_file[path] = []

                            hashline_edits_by_file[path].append(
                                {
                                    "start_line_hash": start_hash,
                                    "end_line_hash": (
                                        end_hash if operation != "insert" else start_hash
                                    ),
                                    "operation": operation,
                                    "text": updated if operation in ["replace", "insert"] else None,
                                }
                            )
                            original_hashline_edits_by_file[path].append(edit)
                            is_hashline = True

                # Check if this is the transformed format: list of dictionaries (from dry run)
                elif (
                    not is_hashline
                    and isinstance(original, list)
                    and len(original) > 0
                    and isinstance(original[0], dict)
                    and "start_line_hash" in original[0]
                ):
                    if path not in hashline_edits_by_file:
                        hashline_edits_by_file[path] = []
                        original_hashline_edits_by_file[path] = []
                    ...
                    for op in original:
                        hashline_edits_by_file[path].append(op)

                    # If it's already transformed, we should ideally keep it as is for dry run
                    # but the issue is that it might have been transformed by a previous dry run
                    # and we want to avoid double-wrapping or losing the "original" nature.
                    original_hashline_edits_by_file[path].append(edit)
                    is_hashline = True
        ...
        # Process hashline edits in batch for each file
        for path, operations in hashline_edits_by_file.items():
        ...
        for edit in regular_edits:
            ...
            if not new_content and (
                (isinstance(original, str) and original.strip())
                or (isinstance(original, list) and len(original) > 0)
            ):
    ...
    while i < len(lines):
        ...
        if head_pattern.match(line.strip()):
            try:
                ...
                updated_text_str = "".join(updated_text)
                ...
                yield filename, original_text_str, updated_text_str
