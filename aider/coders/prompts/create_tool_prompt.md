...
You must implement this tool as a Python module containing a class named `Tool` that inherits from `BaseTool`.
...
class Tool(BaseTool):
    ...
    @staticmethod
    def execute(coder, param1: str, param2: bool = False):
        """
...
2.  Implement a class named `Tool` that inherits from `aider.tools.utils.base_tool.BaseTool`.
3.  Define the JSON schema for the tool as a class attribute named `SCHEMA`.
4.  Implement the tool's functionality in a static method named `execute(coder, ...)`.
5.  The `execute` method's signature must match the parameters defined in the JSON schema, with `coder` as the first argument.
6.  The tool code should not contain any example usage code (e.g. `if __name__ == '__main__':`).
