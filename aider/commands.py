...
class Commands:
    ...
    async def cmd_add(self, args, is_tool_file=False):
        ...
        for matched_file in sorted(all_matched_files):
            ...
            if not is_tool_file:
                try:
                    Path(abs_file_path).relative_to(self.coder.root)
                except ValueError:
                    # File is not within the project root
                    if not is_image_file(matched_file):
                        self.io.tool_error(
    ...
    async def cmd_runprocess(self, args):
        "Execute a command in a separate, isolated subprocess."
        from aider.coders.agent_coder import AgentCoder

        if not isinstance(self.coder, AgentCoder):
            self.io.tool_error("The /runprocess command is only available in agent mode.")
            return

        from aider.tools import run_process

        res = await run_process.Tool.execute(self.coder, args)
